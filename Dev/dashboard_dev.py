...
## Fetching and displaying local feature importance 
# Function to fetch feature importance for a client
def fetch_feature_importance(client_id):
    api_url = f'http://192.168.1.40:5000/feature_importance/{client_id}'
    response = requests.get(api_url)

    if response.status_code == 200:
        data = response.json()
        feature_importance = data.get('local_feature_importance', {})
    else:
        st.error(f"Error fetching feature importance from API. Status code: {response.status_code}")
        st.error(f"Error message from API: {response.text}")
        feature_importance = {}
...
feature_importance = fetch_feature_importance(selected_client_id)

# Function to fetch the feature details from the API
def fetch_details_from_api():
    api_url = 'http://192.168.1.40:5000/get_details'  
    response = requests.get(api_url)

    if response.status_code == 200:
        json_data = response.json()
        df_details = pd.read_json(json_data)  
        return df_details
    else:
...
df_details = fetch_details_from_api()

# Displaying local feature importance 
with col2:
    if feature_importance:
        st.subheader(f'Top 10 Feature Importance for Client {selected_client_id}')
        
        # Sort feature importance by importance values and get the top 10
        sorted_feature_importance = dict(sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)[:10])
        
        for feature, importance in sorted_feature_importance.items():
            st.write(f'{feature}: {importance}')
    else:
        st.error('Feature importance not available for the selected client.')


# Displaying Feature details 
with col2: 
    # Display feature details for the top 10 features
    if df_details is not None:
        st.subheader('Feature Details')
        top_10_features = list(sorted_feature_importance.keys())
        filtered_df = df_details[df_details['Row'].isin(top_10_features)]
        st.write(filtered_df[['Row', 'Description']])
    else:
        st.error('Feature details not available from the API.')
