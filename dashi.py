...
def fetch_client_ids():
    api_url = 'http://192.168.1.40:5000/predict/'  
...
def fetch_and_display_client_features(sk_id):
    ...
    response = requests.get(f'http://192.168.1.40:5000/get_client_features/{sk_id}')
...
### Local feature importance (for each client)
# Function to fetch feature importance for the selected client
def fetch_feature_importance(sk_id, top_n=10):
    api_url = f'http://192.168.1.40:5000/feature_importance/{sk_id}'
    response = requests.get(api_url)

    if response.status_code == 200:
        feature_importance = response.json()
    else:
        st.error(f"Error fetching feature importance. Status code: {response.status_code}")
        feature_importance = {}

    # Sort feature importance by importance score in descending order
    sorted_feature_importance = dict(
        sorted(feature_importance.items(), key=lambda item: item[1], reverse=True)
    )

    # Select the top N features
    top_feature_importance = dict(list(sorted_feature_importance.items())[:top_n])
...
# Load the CSV file into a DataFrame with a specific encoding
feature_details_df = pd.read_csv('Detail_features.csv', encoding='Windows-1252')

# Fetch the top N feature importance for the selected client
top_feature_importance = fetch_feature_importance(selected_client_id, top_n=top_n_features)

# Display top feature importance and their meaning 
with col2:
    # Create a subheader for feature importance
    st.subheader('Top Feature Importance for the Client')
    if top_feature_importance:
        df_top_feature_importance = pd.DataFrame.from_dict(top_feature_importance, orient='index', columns=['Importance'])
        st.write(df_top_feature_importance)

        # Display the descriptions of the top ten features in a table
        st.subheader('Features Details')
        feature_details_list = []
        for feature_name in df_top_feature_importance.index:
            feature_details = feature_details_df.loc[feature_details_df['Row'] == feature_name]
            if not feature_details.empty:
                feature_description = feature_details['Description'].values[0]
                feature_details_list.append([feature_name, feature_description])

        if feature_details_list:
            df_feature_details = pd.DataFrame(feature_details_list, columns=['Feature Name', 'Description'])
            st.write(df_feature_details)
        else:
            st.warning("No details available for the top features.")
    else:
        st.warning("Feature importance data not available for the selected client.")
...
def fetch_dataframe_from_api():
    api_url = 'http://192.168.1.40:5000/get_dataframe'  
    response = requests.get(api_url)
...
def fetch_client_feature_value(client_id, feature_name):
    api_url = f'http://192.168.1.40:5000/client_feature_value/{client_id}/{feature_name}'
    response = requests.get(api_url)
